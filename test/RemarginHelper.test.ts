...
describe("RemarginHelper", () => {
    ...
    it("main", async () => {
        ...
        // await liquidityPool.withdraw(0, user0.address, toWei("200"));
    ...
    it("main - eth", async () => {
        ...
        // await ctk.mint(user0.address, toWei("10000"));
        await weth.connect(user0).approve(liquidityPool.address, toWei("10000"));

        await liquidityPool.deposit(0, user0.address, toWei("0"), { value: toWei("10") });
        // await liquidityPool.withdraw(0, user0.address, toWei("200"));

        const remarginHelper = await createContract("RemarginHelper")
        await poolCreator.grantPrivilege(remarginHelper.address, 0x3)

        var result = await liquidityPool.getMarginAccount(0, user0.address)
        ...
        var result = await liquidityPool.getMarginAccount(1, user0.address)
        expect(result.cash).to.equal(toWei("5"))
