...
contract Perpetual {
    ...
    function price() public view returns (int256, uint256, int256, uint256) {
        return (
            int256(100 ether), 
            block.timestamp, 
            int256(100 ether),
    ...
    function marginAccount(address) public pure returns (int256, int256, int256) {
    ...
    function trade(int256 amount, int256 priceLimit, uint256 deadline) public {
        emit Trade(msg.sender, amount, 120 ether);
    }

    event Trade2(address trader, int256 amount, int256 price, bytes data);
    function trade2(address trader, int256 amount, int256 priceLimit, uint256 deadline, bytes memory data) public {
    ...
    function batchTrade(Order[] calldata orders, int256[] calldata amounts, int256[] calldata gases, FailureOption option) external {
