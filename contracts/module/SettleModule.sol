...
library SettleModule {
	...
	function withdrawableMargin(Core storage core, address trader)
		internal
		returns (int256 amount)
	{
		int256 margin = core.margin(trader);
		if (core.marginAccounts[trader].positionAmount != 0) {
			amount = core.withdrawableMarginWithPosition.wfrac(margin, core.marginWithPosition);
			core.marginWithPosition = core.marginWithPosition.sub(margin);
			core.withdrawableMarginWithPosition = core.withdrawableMarginWithPosition.sub(amount);
		} else {
			...
			core.withdrawableMarginWithoutPosition = core.withdrawableMarginWithoutPosition.sub(
				amount
			);
		}
	}
