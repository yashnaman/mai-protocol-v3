...
library CollateralModule {
    ...
    //  */
    // function __CollateralInitialize(address collateral) internal {
    ...
    //     require(
    //         decimals <= Constant.MAX_COLLATERAL_DECIMALS,
    //         "decimals out of range"
    //     );
    //     _collateral = collateral;
    //     _scaler = uint256(
    //         10**(Constant.MAX_COLLATERAL_DECIMALS.sub(uint256(decimals)))
    //     );
    // }
     ...
     */
    function retrieveDecimals(address tokenAddress)
        internal
        view
        returns (uint8, bool)
    {
     ...
     */
    function collateralBalance(Core storage core, address account)
        internal
        view
        returns (uint256)
    {
        return IERC20(core.collateral).balanceOf(account);
    }
    ...
    function transferFromUser(
        ...
        IERC20(core.collateral).safeTransferFrom(
            account,
            address(this),
            rawAmount
        );
     ...
     */
    function _toRawAmount(Core storage core, uint256 amount)
        private
        view
        returns (uint256)
    {
