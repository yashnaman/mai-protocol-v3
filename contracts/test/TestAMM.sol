...
contract TestAMM {
        ...
        returns (int256 deltaMargin)
    {
        ...
        deltaMargin = AMMTradeModule.longDeltaMargin(
            m0,
            mc.add(mv),
            core.marginAccounts[address(this)].positionAmount,
            positionAmount2,
        ...
        returns (int256 deltaMargin)
    {
        ...
        deltaMargin = AMMTradeModule.shortDeltaMargin(
            m0,
            core.marginAccounts[address(this)].positionAmount,
            positionAmount2,
            core.indexPrice(),
    ...
    function maxLongPosition(int256 beta) public view returns (int256) {
        (, int256 m0) = regress(beta);
        return
            AMMTradeModule._maxLongPosition(m0, core.indexPrice(), beta, core.targetLeverage.value);
    }

    function maxShortPosition(int256 beta) public view returns (int256) {
        (, int256 m0) = regress(beta);
        return
            AMMTradeModule._maxShortPosition(
                m0,
                core.indexPrice(),
                beta,
                core.targetLeverage.value
    ...
    {
        (deltaMargin, deltaPosition) = AMMTradeModule.tradeWithAMM(
            core,
            tradingAmount,
            partialFill
        );
